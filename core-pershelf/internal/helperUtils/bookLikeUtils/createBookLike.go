...
func CreateBookLike(bookLike tablesModels.BookLike) (tablesModels.BookLike, error) {
	...
	jsonData, err := helperContact.HelperRequest("/likes/create", payload)
	if err != nil {
		log.Printf("Error calling helper request: %v", err)
		return tablesModels.BookLike{}, fmt.Errorf("error creating book like: %v", err)
	}

	var likeResp response.BookLikesResp
	if err := json.Unmarshal(jsonData, &likeResp); err != nil {
		log.Printf("Error unmarshalling response: %v", err)
		return tablesModels.BookLike{}, fmt.Errorf("error unmarshalling created book like: %v", err)
	}

	if likeResp.Status.Code != "0" {
		log.Printf("Backend returned error: %s", likeResp.Status.Code)
		return tablesModels.BookLike{}, fmt.Errorf("error creating book like: %s", likeResp.Status.Code)
	}

	if len(likeResp.BookLikes) == 0 {
		log.Printf("Book like creation succeeded but no book like returned")
		return tablesModels.BookLike{}, fmt.Errorf("no book like returned after creation")
	}
