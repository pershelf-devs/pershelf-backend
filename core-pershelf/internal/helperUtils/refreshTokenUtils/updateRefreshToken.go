...
func UpdateRefreshToken(refreshToken tablesModels.RefreshToken) (tablesModels.RefreshToken, error) {
	...
	jsonData, err := helperContact.HelperRequest("/refreshTokens/update", payload)
	if err != nil {
		log.Printf("Error calling helper request: %v", err)
		return tablesModels.RefreshToken{}, fmt.Errorf("error updating refresh token: %v", err)
	}

	var tokenResp response.RefreshTokensResp
	if err := json.Unmarshal(jsonData, &tokenResp); err != nil {
		log.Printf("Error unmarshalling response: %v", err)
		return tablesModels.RefreshToken{}, fmt.Errorf("error unmarshalling updated refresh token: %v", err)
	}

	if tokenResp.Status.Code != "0" {
		log.Printf("Helper microservice returned an error with code: : %s", tokenResp.Status.Code)
		return tablesModels.RefreshToken{}, fmt.Errorf("error updating refresh token: %s", tokenResp.Status.Code)
	}

	if len(tokenResp.RefreshTokens) == 0 {
		log.Printf("Refresh token update succeeded but no token returned")
		return tablesModels.RefreshToken{}, fmt.Errorf("no refresh token returned after update")
	}
